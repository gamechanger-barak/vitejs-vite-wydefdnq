...
const GreenTeamWins: React.FC<GreenTeamWinsProps> = ({ gameData, roomId, isHost }) => {
  ...
  // הדפסה ל-Console כדי שתוכל לראות מה באמת מגיע מה-DB
  useEffect(() => {
    console.log("DEBUG: GreenTeamWins received gameData:", gameData);
  }, [gameData]);

  // חילוץ נתונים בטוח - מוודא שתמיד נקבל מערך, גם אם ה-JSON שבור
  const questions = useMemo(() => {
    if (!gameData) return [];
    if (Array.isArray(gameData)) return gameData;
    if (typeof gameData === 'object' && gameData.content && Array.isArray(gameData.content)) {
  ...
  const metadata = useMemo(() => {
    const defaultMeta = { company_name: "TEAM BUILDING", theme: "Green Team Wins" };
    if (!gameData || Array.isArray(gameData)) return defaultMeta;
    return {
      company_name: gameData.game_metadata?.company_name || defaultMeta.company_name,
      theme: gameData.game_metadata?.theme || defaultMeta.theme
    };
  }, [gameData]);

  // הגנה קריטית: אם אין שאלות, אל תנסה לרנדר את המשחק
  if (!questions || questions.length === 0) {
    return (
      <div className="min-h-screen bg-[#05050a] text-white flex flex-col items-center justify-center font-sans" dir="rtl">
        <div className="w-12 h-12 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mb-4" />
        <p className="text-emerald-500 font-bold tracking-widest uppercase animate-pulse">טוען נתונים מהשרת...</p>
        <p className="text-gray-600 text-xs mt-2">Room ID: {roomId}</p>
      </div>
    );
  }

  const currentCard = questions[currentIndex] || { prompt: "שגיאה בטעינת שאלה", category: "כללי" };
  ...
  if (phase === 'START') {
    return (
      <div dir="rtl" className="min-h-screen bg-[#05050a] text-white p-12 flex flex-col items-center justify-center font-sans overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-emerald-600/10 rounded-full blur-[150px]" />
        <div className="text-center relative z-10 space-y-12">
          ...
          <button 
            ...
            className="px-12 py-5 bg-emerald-600 text-black rounded-2xl font-bold text-2xl transition-all shadow-[0_0_30px_rgba(16,185,129,0.4)] hover:bg-emerald-400 hover:scale-105 active:scale-95 cursor-pointer"
          >
            כניסה למערכת
          </button>
        </div>
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-[#05050a] text-white p-12 flex flex-col items-center justify-center font-sans">
        <div className="relative z-10 text-center space-y-12 w-full max-w-4xl">
          <PartyPopper className="w-24 h-24 text-amber-400 mx-auto" />
          ...
          <button onClick={() => window.location.reload()} className="flex items-center gap-3 text-gray-500 hover:text-white transition-all mx-auto pt-8 group cursor-pointer">
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#05050a] text-white p-8 md:p-16 font-sans relative flex flex-col overflow-hidden">
            ...
            <div className="text-center">
              ...
              <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className="w-10 h-10 rounded-xl bg-emerald-500 text-black hover:bg-emerald-400 transition-all flex items-center justify-center cursor-pointer shadow-lg active:scale-90"
                >
      ...
      <main className="relative z-20 flex-1 flex flex-col justify-center max-w-5xl mx-auto w-full">
        <div className="mb-12 space-y-3">
          ...
          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
            <div 
              className="h-full bg-gradient-to-r from-emerald-600 to-cyan-500 transition-all duration-700 shadow-[0_0_20px_rgba(16,185,129,0.5)]"
        ...
        <div className="relative bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl rounded-[2.5rem] p-16 md:p-24 min-h-[480px] flex flex-col justify-center items-center text-center relative overflow-hidden shadow-2xl">
          ...
          <h2 className="text-4xl md:text-6xl font-bold leading-[1.1] tracking-tight max-w-4xl transition-all duration-500">
            {currentCard.prompt}
          </h2>

          {phase === 'REVEAL' && (
            <div className="mt-16 transform transition-all duration-500 scale-105">
        ...
        {isHost && (
          <div className="mt-16 flex justify-center">
            <button 
              onClick={nextStep}
              className="group relative flex items-center gap-6 px-16 py-6 bg-white text-black rounded-[2rem] font-black text-2xl transition-all hover:bg-emerald-400 shadow-2xl active:scale-95 cursor-pointer"
            >
      ...
      <footer className="mt-12 text-center opacity-20">
        <div className="text-[10px] font-mono uppercase tracking-[0.3em]">Room: {roomId} • Platform: 2.1.0</div>
      </footer>
    </div>
  );
};
